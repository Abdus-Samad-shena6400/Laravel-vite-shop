    ...
    <div class="p-6">
        ...
        <div v-if="loading" class="bg-white rounded-xl shadow p-20 text-center">
            <svg class="animate-spin h-10 w-10 mx-auto text-indigo-600" xmlns="http://www.w3.org/2000/svg" fill="none"
                ...
                <path class="opacity-75" fill="currentColor" d="M 12 2 A 10 10 0 0 0 2 12 A 10 10 0 0 0 12 22 A 10 10 0 0 0 22 12 A 10 10 0 0 0 12 2 z" />
            </svg>
        ...
        <div v-else class="bg-white rounded-xl shadow p-6">
            <form @submit.prevent="submitForm">
                ...
                <div class="md:col-span-2">
                    ...
                    <img v-else-if="product.featured_image" :src="'http://localhost:8000/storage/' + product.featured_image" class="mt-4 w-40 h-40 object-cover rounded-lg border">
                ...
                <div class="md:col-span-2 mt-6">
                    ...
                    <div v-if="product.images && product.images.length > 0" class="mt-4 grid grid-cols-4 gap-4">
                        <div v-for="image in product.images" :key="image.id" class="relative">
                            <img :src="'http://localhost:8000/storage/' + image.image" class="w-full h-32 object-cover rounded-lg border">
                            <button type="button" @click="removeExistingGalleryImage(image.id)"
                    ...
                    <div v-if="galleryImages.length > 0" class="mt-4 grid grid-cols-4 gap-4">
                        <div v-for="(image, index) in galleryImages" :key="'new-' + index" class="relative">
...
const removeExistingGalleryImage = (imageId) => {
    removeGalleryImages.value.push(imageId)
...
const submitForm = async () => {
    ...
    try {
        ...
        if (removeGalleryImages.value.length > 0) {
            removeGalleryImages.value.forEach((id, index) => {
                formData.append(`remove_gallery_images[${index}]`, id)
            })
        }

        await axiosClient.post(`/products/${route.params.id}`, formData, {
            headers: {
